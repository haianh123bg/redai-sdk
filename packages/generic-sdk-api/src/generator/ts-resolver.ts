...
export const resolveTsOperations = (params: {
  ...
  for (const sf of program.getSourceFiles()) {
    ...
    const controllerDomain = path.basename(sf.fileName, '.controller.ts').toLowerCase();
...
export const resolveTsRoutes = (params: {
  ...
  for (const op of ops) {
    result.set(`${op.method.toUpperCase()} ${op.path}`, {
      ...
      controllerDomain: op.controllerDomain,
    });
  }
  return result;
};
...
export const resolveTsTypeDeclarations = (params: {
  ...
  const { backendRoot, moduleName, requestedTypeNames } = params;
  ...
  const program = createProgram(backendRoot, files);
  const checker = program.getTypeChecker();
  const declMap = new Map<string, Array<{ node: ts.Node; sourceFile: ts.SourceFile }>>();
  const moduleMarker = `/modules/${moduleName.toLowerCase()}/`;
  ...
  for (const sf of program.getSourceFiles()) {
    ...
    for (const st of sf.statements) {
      if (
        ...
        const list = declMap.get(st.name.text) ?? [];
        list.push({ node: st, sourceFile: sf });
        declMap.set(st.name.text, list);
      }
    }
  }

  const pickPreferredDeclaration = (
    declarations: Array<{ node: ts.Node; sourceFile: ts.SourceFile }>,
  ): { node: ts.Node; sourceFile: ts.SourceFile } => {
    const sorted = [...declarations].sort((a, b) => {
      const aPath = a.sourceFile.fileName.replace(/\\/g, '/').toLowerCase();
      const bPath = b.sourceFile.fileName.replace(/\\/g, '/').toLowerCase();
      const aIsModule = aPath.includes(moduleMarker);
      const bIsModule = bPath.includes(moduleMarker);
  ...
  while (queue.length > 0) {
    ...
    seen.add(name);
    const targets = declMap.get(name);
    if (!targets || targets.length === 0) continue;
    const target = pickPreferredDeclaration(targets);
